 ...
 * @since chapter12.01 Added http.sessionManagement().sessionFixation().none();
 * @since chapter12.02 Set http.sessionManagement().sessionFixation() to .migrateSession()
 * @since chapter12.02 Set maximum active sessions: http.sessionManagement().maximumSessions(1);
 * @since chapter12.03 Configuring expired session redirect
 * @since chapter12.04 Added .maxSessionsPreventsLogin(true) for Preventing authentication instead of forcing logout
 * @since chapter12.05 Updated .maximumSessions(2)
 */
@Configuration
